...
const index = require('./shared/eslint-config-vuetify.DvbgKdIP.cjs');
const gitignoreConfig = require('eslint-config-flat-gitignore');
const perfectionistPlugin = require('eslint-plugin-perfectionist');
const eslintPluginPnpm = require('eslint-plugin-pnpm');
const tseslintVendor = require('typescript-eslint');
const unicornVendor = require('eslint-plugin-unicorn');
const vueVendor = require('eslint-plugin-vue');
...
const gitignoreConfig__default = /*#__PURE__*/_interopDefaultCompat(gitignoreConfig);
const perfectionistPlugin__default = /*#__PURE__*/_interopDefaultCompat(perfectionistPlugin);
const tseslintVendor__default = /*#__PURE__*/_interopDefaultCompat(tseslintVendor);
const unicornVendor__default = /*#__PURE__*/_interopDefaultCompat(unicornVendor);
const vueVendor__default = /*#__PURE__*/_interopDefaultCompat(vueVendor);
const vueParser__namespace = /*#__PURE__*/_interopNamespaceCompat(vueParser);
const jsoncParser__namespace = /*#__PURE__*/_interopNamespaceCompat(jsoncParser);
const yamlParser__namespace = /*#__PURE__*/_interopNamespaceCompat(yamlParser);
...
async function antfu(options = true) {
  ...
  const antfuPlugin = await index.interopDefault(import('eslint-plugin-antfu'));
  return [
    {
      name: "vuetify/antfu",
      ...filesConfig,
...
function autoimports(options = true) {
  const autoimports2 = index.loadAutoImports(options);
  return {
...
function gitignore(options = true) {
  ...
  return gitignoreConfig__default({
...
function ignore(options) {
  ...
  return config.globalIgnores([...ignoreList, ...extendIgnoreList], "vuetify/ignore");
}
...
async function imports(options = true) {
  ...
  if (pluginType === "import-x") {
    await index.assertPackage("eslint-plugin-import-x", 'import: { plugin: "import-lite" }');
  ...
  return [
    {
      ...
      ...filesConfig,
      plugins: { perfectionist: perfectionistPlugin__default },
      rules
    }
  ];
}

const hasWorkspace = index.hasFile("pnpm-workspace.yaml");
  ...
  return [
    {
      ...
      languageOptions: { parser: jsoncParser__namespace },
      name: "vuetify/pnpm/package-json",
      plugins: { pnpm: eslintPluginPnpm.plugin },
      rules: {
    ...
    },
    {
      ...
      languageOptions: { parser: yamlParser__namespace },
...
async function stylistic(options = true, optionsVue = false) {
  ...
  const stylisticPlugin = await index.interopDefault(import('@stylistic/eslint-plugin'));
  const stylistic2 = [
    {
...
async function test(options = true) {
  if (!options) {
    ...
    if (!index.hasPackage("@vitest/eslint-plugin") && !index.hasPackage("eslint-plugin-jest")) {
      return {};
    } else {
      options = { files: GLOB_TESTS, runner: index.hasPackage("@vitest/eslint-plugin") ? "vitest" : "jest" };
    }
  }
  const files = options?.files ?? GLOB_TESTS;
  const runner = options?.runner;
  if (runner === "vitest") {
    await index.assertPackage("@vitest/eslint-plugin", "test: false");
    const vitestVendor = await index.interopDefault(import('@vitest/eslint-plugin'));
    const noOnlyTests = await index.interopDefault(import('eslint-plugin-no-only-tests'));
    const config = [
      {
        files,
        plugins: {
  ...
  if (runner === "jest") {
    await index.assertPackage("eslint-plugin-jest", "test: false");
    const jestVendor = await index.interopDefault(import('eslint-plugin-jest'));
    const noOnlyTests = await index.interopDefault(import('eslint-plugin-no-only-tests'));
    const config = [
      {
        files,
...
function typescriptCore(preset) {
  return tseslintVendor__default.config({
    extends: [...tseslintVendor__default.configs[preset]],
...
function unicorn(options = true) {
  ...
  return [
    {
      ...filesConfig,
      ...unicornVendor__default.configs["recommended"],
...
const recommendedRules = vueVendor__default.configs["flat/recommended"].map((c) => c.rules).reduce((acc, c) => ({ ...acc, ...c }), {});
const rules = {
  ...recommendedRules,
...
async function vue(options = true, tsOptions = true) {
  const plugins = { vue: vueVendor__default };
  ...
  const languageOptions = {
    parser: vueParser__namespace,
    parserOptions: {
      ecmaFeatures: { jsx: true },
      extraFileExtensions: [".vue"],
      sourceType: "module",
      parser: tseslintVendor__default.parser
    }
  };
  if (tsEnabled) {
    plugins["@typescript-eslint"] = tseslintVendor__default.plugin;
  }
  if (typeof options === "object" && options?.a11y) {
    await index.assertPackage("eslint-plugin-vuejs-accessibility", "vue.a11y: false");
    const a11yVendor = await index.interopDefault(import('eslint-plugin-vuejs-accessibility'));
  ...
  return [
    ...
    ...tsConfig,
    {
      ...
      name: "vuetify/vue",
      plugins,
      processor: vueVendor__default.processors[".vue"],
      rules: {
        ...rules,
...
const isInEditorSchema = exactOptional(boolean(), index.isInEditorEnv());
const typescriptSchema = exactOptional(
  union([
    ...
    object({
      ...
      projectService: exactOptional(boolean())
    })
  ]),
  index.hasPackage("typescript")
);
const vueSchema = exactOptional(
  ...
  index.hasPackage("vue") || index.hasPackage("@vue/compat")
);
...
const stylisticSchema = exactOptional(
  union([
    boolean(),
    object({
      files: exactOptional(array(string())),
      severity: exactOptional(union([literal("error"), literal("warn")]))
    })
  ]),
  index.hasPackage("typescript")
);
...
const importsSchema = exactOptional(
  union([
    boolean(),
    object({
      files: exactOptional(array(string())),
      plugin: exactOptional(union([literal("import-lite"), literal("import-x")]), "import-lite")
    })
  ]),
  true
);
const testSchema = exactOptional(
  ...
  index.hasPackage("jest") || index.hasPackage("vitest")
);
...
const pnpmSchema = exactOptional(
  union([
    ...
    object({
      files: exactOptional(array(string())),
      enforceCatalog: exactOptional(boolean())
    })
  ])
);
...
async function buildConfig(maybeOptions, ...userConfigs) {
  ...
  const pnpmEnabled = vOptions.pnpm === void 0 ? (await index.getPackageManager())?.name === "pnpm" : Boolean(vOptions.pnpm);
  if (pnpmEnabled) {
  ...
  let composed = eslintFlatConfigUtils.composer(await eslintFlatConfigUtils.concat(
    ...configsToCompose,
    ...userConfigs
  ));
